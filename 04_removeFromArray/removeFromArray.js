const removeFromArray = function(array, ...target) {
    ...
    // (item) => !target.includes(item)
    let result = array.filter(function(item) {return !target.includes(item)}  ); // anonymous callback function
              // array.filter((item)  =>  !target.includes(item)  ); arrow callback function
    return result;
};
...
const removeFromArray = function (array, ...args) {
    ...
    const newArray = [];
    // use f ach to go through the array
    array.forEach((item) => {
